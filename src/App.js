...
function App() {
    ...
    return ( <
        ...
        value = { accountKey }
        onChange = {
            (e) => setAccountKey(e.target.value) }
        ...
        <
        a href = { `${backendUrl}/download-sample` }
        ...
        type = "file"
        multiple onChange = { handleAttachments }
        className = "hidden" /
        >
        <
        div className = "mt-4 grid grid-cols-2 md:grid-cols-3 gap-4" > {
            attachments.map((att, index) => ( <
                div key = { index }
                className = "bg-gray-50 border border-gray-200 rounded-lg p-2 relative text-center" >
                <
                button onClick = {
                    () => handleRemoveAttachment(att.file.name) }
                ...
                title = { att.file.name } > { att.file.name } <
                /p> <
                /div>
            ))
        } <
